...
class MainController extends Controller
{
    ...
    {
        $userId = (isset($_POST['currentUser'])) ? $_POST['currentUser'] : Yii::app()->usuario->getId();
        Yii::app()->usuario->setState('calUserId', $userId);
    ...
    {
        if ((Yii::app()->request->isAjaxRequest) and (Yii::app()->usuario->hasState('calUserId')) )
        {
            $criteria = new CDbCriteria(array(
                        'condition' => 'user_id=:user_id',
                        'params'=>array(':user_id'=> Yii::app()->usuario->getState('calUserId')),
                    ));
    ...
    {
        if (!Yii::app()->usuario->hasState('calUserId'))
            Yii::app()->end();

        $user_id = Yii::app()->usuario->getState('calUserId');
        $title = $_POST['title'];
        $start = $_POST['start'];
        $end = $_POST['end'];
    ...
    public function actionUserpreference()
    {
        if (Yii::app()->request->isAjaxRequest)
        {
            if (!Yii::app()->usuario->hasState('userId'))
                Yii::app()->end();

            $userPref = EventsUserPreference::model();
            $userPref->attributes = $_POST['EventsUserpreference'];
            $userPref->updateByPk(Yii::app()->usuario->getState('calUserId') /*$userPref->attributes['user_id']*/, $_POST['EventsUserpreference']);
            Yii::app()->end();
        }
    }
